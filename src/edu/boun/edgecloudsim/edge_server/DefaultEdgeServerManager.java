...
public class DefaultEdgeServerManager extends EdgeServerManager{
	...
	public void startDatacenters() throws Exception{
                SimLogger SS = SimLogger.getInstance(); //to 
		...
		for (int i = 0; i < datacenterList.getLength(); i++) {
			...
			localDatacenters.add(createDatacenter(i, datacenterElement));
                        //calculating the idleEnergyConsumption
                        //obtaining the hosts of the datacenter
...
// 2. A Machine contains one or more PEs or CPUs/Cores. Therefore, should
        ...
        public double calculateIdleEnergyConsumption(Host host){
                //obtaining variables
                double energyConsumption;
                double simulationRemainingTime = SimSettings.getInstance().getSimulationTime() - CloudSim.clock();
                //idle energy consumption = maxEnergyConsumption * alfa
                energyConsumption = host.getmaxEnergyConsumption() * host.getIdleEnergyConsumption()*simulationRemainingTime;
